  ...
  render() {
    ...
    return (
      <div>
        <div />
        <div>
          <h2> {data.map((item, ikey) => item.title)} </h2>
        </div>
        <div className="row">
          ...
          item.domains.map((domain, domainkey) => (
            <div key={domainkey}>
              <div className="row callout" key={domain.id}>
                <div className="column large-2">
                  <p className="badge">
                    {this.state.framework[2][0].toUpperCase() +
                      '' +
                      (domainkey + 1)}
                  </p>{' '}
                </div>
              ...
              {domain.competencies.map((competency, ckey) => (
                <div className="row" key={ckey}>
                  <div className="row" key={competency.id}>
                    <div className="column large-2 light-gray-background">
                      <p className="badge">
                        {domainkey + 1 + '.' + (ckey + 1)}{' '}
                      </p>{' '}
                    </div>
                  ...
                  <div className="row">
                    {frameworkDefs.map((def, defindex) => {
                      return (
                        <div className="row" key={defindex}>
